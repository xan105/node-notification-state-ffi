...
function load(name, option = {}){
  ...
  try{
    ...
    lib = ffi.DynamicLibrary(name, FLAGS[options.mode] ?? FLAGS.RTLD_NOW);
  }catch(err){
...
function dlopen(name, fn){
  
  shouldObj(fn);
  
  let lib = {};
  const call = load(name);
  for (const [fnSymbol, [resultType, paramType]] of Object.entries(fn))
    lib[fnSymbol] = call(fnSymbol, resultType, paramType);
  return lib;
}
