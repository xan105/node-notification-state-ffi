...
import { dlopen, types } from "@xan105/ffi/koffi";
...
koffi.alias("HRESULT", types.HRESULT);
koffi.alias("ENUM", types.ENUM);

console.log(koffi.types);

const { SHQueryUserNotificationState } = dlopen("shell32.dll", {
  "SHQueryUserNotificationState": {
    result: "HRESULT",
    parameters: [koffi.out(koffi.pointer("ENUM"))],
    nonblocking: true
  }
}, { abi: "stdcall" });
...
async function queryUserNotificationState(option = {}){
  ...
  let pquns = Buffer.alloc(koffi.sizeof("ENUM"));
